...
import { recoverServer } from "@/lib/auto-recovery";
...
export async function POST(_req: NextRequest, ctx: Ctx) {
  try {
    ...
    const result = await recoverServer(id, "manual");

    if (result.reason === "not_found") {
      return Response.json({ error: "Not found" }, { status: 404 });
    }
    if (result.reason === "running_tasks_present") {
      return Response.json({ error: "Cannot recover: server has running tasks" }, { status: 409 });
    }
    if (!result.attempted) {
    ...
    return serverError("servers/[id]/recover POST", err);
  }
}
