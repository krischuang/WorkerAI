    ...
    <div className="p-4 md:p-6 max-w-5xl">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 mb-6">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 mb-3">
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Entity Type</label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Entity ID</label>
            <input
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Event Type</label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Since</label>
            <input
              type="datetime-local"
              value={filterSince}
      ...
      {events.length === 0 && !loading && (
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 p-8 text-center">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">No events found matching the current filters.</p>
        </div>
      )}

      {events.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden overflow-x-auto">
          <table className="w-full text-sm min-w-[480px]">
            <thead className="bg-zinc-50 dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-700">
              <tr>
                <th className="px-4 py-2 text-left text-xs font-medium text-zinc-600 dark:text-zinc-400">Event</th>
                <th className="hidden md:table-cell px-4 py-2 text-left text-xs font-medium text-zinc-600 dark:text-zinc-400">Entity</th>
                <th className="hidden md:table-cell px-4 py-2 text-left text-xs font-medium text-zinc-600 dark:text-zinc-400">Actor</th>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {events.map((ev) => (
                <tr key={ev.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800 dark:bg-zinc-950">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className={`inline-block w-2 h-2 rounded-full shrink-0 ${EVENT_DOT[ev.eventType] ?? "bg-zinc-300"}`} />
                      <span className="font-mono text-xs text-zinc-800 dark:text-zinc-200">{ev.eventType}</span>
                    </div>
                  </td>
                  <td className="hidden md:table-cell px-4 py-3">
                    <span className="text-xs text-zinc-500 mr-1">{ev.entityType}</span>
                    <span className="font-mono text-xs text-zinc-700 dark:text-zinc-300">{ev.entityId.slice(-8)}</span>
                  </td>
                  <td className="hidden md:table-cell px-4 py-3 text-xs text-zinc-600 dark:text-zinc-400">{ev.actorType}</td>
