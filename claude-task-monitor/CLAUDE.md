...
The WebSocket server (`ws-server.ts`) requires a valid `__auth` session cookie (the same SHA-256 token that `middleware.ts` validates for the REST API). Connections without a matching cookie are rejected with close code 1008. If `AUTH_SECRET` is unset, the check is skipped with a warning — matching middleware behaviour for first-time setup. In addition, connections are accepted only when the `Origin` header is absent (direct calls), is `localhost`, or has a hostname that matches the HTTP `Host` header (same-hostname policy). This prevents cross-site WebSocket hijacking because a browser cannot forge an `Origin` that matches a different host. See the security review comment at the top of the origin-check block in `ws-server.ts` for full rationale. The REST API enforces `isLocalOrigin` on sensitive routes (stricter: localhost IPs + explicit `ALLOWED_ORIGINS` list).
