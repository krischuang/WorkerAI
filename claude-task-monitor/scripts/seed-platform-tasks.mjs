...
Health score formula: SSH responds +40, tmux present +30, Claude online +20, latency <500ms +10.`,
    priority: "P1",
    taskType: "coding",
...
Scheduler: After health checks — for offline workers with autoRecovery=true and attempts < max, attempt launchClaudeInTmux().
Only recover workers with no running tasks to avoid session corruption.`,
    priority: "P1",
    taskType: "coding",
    estimatedCostLevel: "medium",
  },
...
Scheduler: detectZombieTasks() every poller cycle — two-cycle confirmation before failing to avoid false positives.
Default thresholds: stall_threshold_minutes=30, confirm_cycles=2.`,
  ...
  },
  {
    ...
    description: `Automatically retry failed tasks according to configurable per-task policies: maxRetries count, delay schedule (2min → 5min → 10min capped), and failure reason filtering. Non-retryable reasons (task_not_dispatchable, no_resource) excluded. Add retry count badge on task cards and Retry button on failed task detail pages.
...
Queue advance: skip tasks where retryAfter > now.`,
  ...
  },
  {
    ...
    description: `Enforce a hard wall-clock timeout per task. Resolution order: task.timeoutMinutes → server/agent defaultTaskTimeoutMinutes → SystemConfig global default (120 min). Auto-fail tasks that exceed their timeout; log errorMessage = "Execution timeout (Xmin)"; kill the per-task tmux session. Show elapsed/timeout progress bar on running task detail page.

DB: Task.timeoutMinutes (shared with Zombie Detection), defaultTaskTimeoutMinutes on Server and Agent.
API: timeoutMinutes accepted in POST /api/tasks and PUT /api/tasks/[id]; timeoutExpiresAt computed field returned in GET /api/tasks/[id].
Scheduler: In running-task scan loop — read startedAt from latest ExecutionLog, compare against resolved timeout. Use startedAt (accurate), not updatedAt.
Build this task first — it is the simplest, highest-value change in P0.`,
    priority: "P1",
    taskType: "coding",
    estimatedCostLevel: "low",
  },
  ...
  // ── P1 Queue & Capacity Management ─────────────────────────────────────────
  {
    ...
    description: `Allow tasks to declare prerequisite tasks. Dependent tasks stay in pending until all prerequisites reach completed or archived. Use a denormalized blockedByCount field for fast queue filtering. Enforce cycle detection (DFS) on every new dependency edge before inserting. Unblock dependents atomically when a task completes.
...
Queue advance: add blockedByCount: 0 filter to findFirst.`,
  ...
  },
  {
    ...
    description: `Compute and store each project's completion percentage and task distribution in real time. Calculate a basic velocity-based ETA from tasks completed in the last 7 days. Show progress bar with percentage and estimated completion date on project list and detail pages.
...
API: GET /api/projects/[id] returns progress fields; POST /api/projects/[id]/recalculate; GET /api/projects/[id]/velocity (tasks/day for last 14 days).
Scheduler: Call recalculateProjectProgress(projectId) after every task status change; full reconciliation every 10 poller cycles (~10 min) to prevent drift.
Formula: completionPct = (completedTasks + archivedTasks) / totalTasks * 100.`,
  ...
  },
  {
    ...
    description: `Replace the simple priority+createdAt queue ordering with a multi-factor score. Recompute scores in bulk before each queue advance dispatch using a single SQL UPDATE with CASE WHEN for efficiency.
...
Score formula:
  ...
  queueScore = base + ageBonus + costPenalty + depPenalty + affinityBonus
...
Capacity formula:
  ...
  capacityScore = sum of above  // 0–100

DB: Add capacityScore + activeTaskCount + maxConcurrentTasks (default 10) + capacityUpdatedAt to Server.
API: GET /api/servers/capacity (all servers with scores), GET /api/servers/[id]/capacity (detailed breakdown).
Scheduler: Compute and persist capacity for all servers at the start of each poller cycle. Update activeTaskCount from live running-task count to prevent drift.`,
    priority: "P2",
    taskType: "coding",
    estimatedCostLevel: "medium",
  },
  {
    ...
    description: `When a server or agent hits the 90% usage threshold, set pausedDueToUsage=true and create a ScheduledResume record targeting the exact quota reset time stored in claudeSessionResetsAt / claudeWeekResetsAt. A fast poller step fires due resumes the moment the reset time arrives — eliminating 60+ wasted dispatch skip cycles per quota period.
...
Scheduler: On usage-blocked dispatch — upsert ScheduledResume; triggerDueResumes() step every cycle queries ScheduledResume where resumeAt <= now AND triggered=false.
UI: Resume countdown timer on server/agent cards and detail pages.`,
  ...
  // ── P2 Observability ────────────────────────────────────────────────────────
  {
    ...
    description: `Immutable, append-only event log for every meaningful state change across tasks, projects, servers, and agents. Add emitAudit(event) helper in lib/audit.ts; call it from task-service.ts, task-dispatch.ts, instrumentation.node.ts, and API routes. This is a foundational data layer that all analytics and the improvement engine depend on.
...
UI: Timeline tab on task detail, activity feed on project page, /admin/audit search page.`,
  ...
  },
  {
    ...
    description: `Enrich ExecutionLog with structured metadata captured at completion time: durationMs, exitReason (completion_marker / idle_fallback / timeout / manual / error), paneCapture (last 200 lines of tmux output), retryNumber, failureReason. Add PostgreSQL tsvector full-text search over logText and outputSummary. Nightly archival of logs older than 90 days.

DB: Extend ExecutionLog with durationMs + exitReason + paneCapture + retryNumber + failureReason + archivedAt + searchVector (tsvector); GIN index on searchVector for fast full-text queries.
...
Scheduler: Store paneCapture at completion in poller; nightly archival step at 1 AM UTC (set archivedAt, compress logText to null, keep outputSummary).`,
    priority: "P2",
    taskType: "coding",
    estimatedCostLevel: "medium",
  },
  {
    ...
    description: `Pre-defined reusable task templates with {{variable}} placeholder substitution. Ship 5 built-in templates: Code Review, Write Tests, Fix Bug, Update Documentation, Refactor. Allow operators to create custom templates. "New Task from Template" button on project pages opens a variable substitution form. Built-in templates cannot be edited or deleted.
...
Variable substitution: safe regex-based {{key}} replacement — no eval, no template engines.
UI: /task-templates browse page, "Create from Template" modal with variable form.`,
    priority: "P3",
    taskType: "coding",
    estimatedCostLevel: "low",
  },
  {
    ...
    description: `Generate daily operational reports automatically via the poller at 1 AM UTC, without requiring a manual POST /api/reports/daily. Refactor existing generation logic into a shared generateDailyReport() service function called by both the poller (auto) and the existing manual API route.
...
API: GET /api/reports/daily/[date] (by calendar date YYYY-MM-DD), GET /api/reports/daily/latest.
Scheduler: runDailyReportIfNeeded() in poller — check if today's UTC-date report exists; if not and hour >= 1, generate with generatedBy="auto".
...
DB: Add WeeklyAnalytics model (weekStart + weekEnd + all metric counters); unique on weekStart.
API: GET /api/analytics/weekly (last 12 weeks), GET /api/analytics/weekly/[weekStart] (ISO date), GET /api/analytics/summary (rolling 30-day KPIs).
...
UI: /analytics page with completion bar chart, failure rate trend, duration trend, and KPI summary.`,
    priority: "P3",
    taskType: "coding",
    estimatedCostLevel: "medium",
  },
  ...
  // ── P3 Autonomous Improvement ───────────────────────────────────────────────
  {
    ...
    description: `After a task completes, automatically queue it for a structured Claude quality review with a 5-minute grace delay. Gate progression to archived on a "done" verdict; reset to pending with reviewStatus=incomplete on a failing verdict. Process max 3 reviews per poller cycle to avoid blocking dispatch. Extends existing reviewTask() in task-service.ts to support agent sessions.
...
Scheduler: After task completion — if autoReviewEnabled, set reviewStatus=pending, reviewScheduledAt=now+5min. processReviewQueue() step: find pending reviews due, set in_progress, call reviewTask(), update status.
Guard: do not run review if server has other running tasks (existing runningCount > 0 guard).`,
    priority: "P3",
    taskType: "coding",
    estimatedCostLevel: "medium",
  },
  {
    ...
    description: `Periodically compile all completed task summaries for a project and send them to Claude for a holistic gap analysis: unmet goals, coverage gaps, architectural patterns, and suggested next tasks. Parse structured JSON findings from the Claude response. Truncate to last 50 task summaries for context safety. Findings feed the Backlog Generator.
...
Scheduler: Weekly runDueProjectScans() — for active projects with autoScanEnabled=true, completionPct > 50%, and lastScannedAt + scanFrequencyDays < now.
Scan prompt: XML-fenced task summaries via prompt-sanitiser.ts extension; require JSON output { findings: [{ type, title, severity, description, suggestedAction }] }.`,
    priority: "P3",
    taskType: "coding",
    estimatedCostLevel: "high",
  },
  {
    ...
    description: `Run a specialised debt-detection scan against a project's task history to surface TODO/FIXME patterns, deferred tests, hardcoded values, missing error handling, undocumented APIs, security shortcuts, and performance anti-patterns. Store findings as DebtItem records with severity and category. Provide a living debt register with trend tracking.
...
API: GET /api/projects/[id]/debt (filter by status/severity), PUT /api/debt/[id] (update status/link resolution task), GET /api/debt?status=open&severity=high.
UI: Debt Register tab on project detail; severity-coloured items; "Create task to resolve" button; open debt count badge on project cards; debt trend chart (open items over 8 weeks).
Reuses ProjectScan infrastructure with scanType="debt".`,
  ...
  },
  {
    ...
    description: `Convert scan findings, debt items, and review verdicts into structured TaskSuggestion records. Suggestions sit in a human-approval staging area before becoming real Task rows. Cap at 10 suggestions per scan. De-duplicate against existing open suggestions. Provide bulk approve/reject UI with edit-before-approve capability.
...
API: GET /api/projects/[id]/suggestions (filter by status), POST /api/suggestions/[id]/approve (creates Task, sets status=converted), POST /api/suggestions/[id]/reject, PUT /api/suggestions/[id] (edit before approving), POST /api/projects/[id]/generate-suggestions.
Scheduler: After ProjectScan completes — call generateSuggestionsFromScan(scanId); after DebtItem created — call generateSuggestionFromDebt(debtItemId).`,
    priority: "P3",
    taskType: "coding",
    estimatedCostLevel: "medium",
  },
